...
export default class RTCPeerConnection extends EventTarget(
  ...
  addStream(stream: MediaStream) {
    // FLAG: callback으로 바꿨으니 callback으로 해주긴 하는데 왜 callback일까?
  ...
  // FLAG: 추가함.
  addTrackV1(track: MediaStreamTrack) {
    // Version 1
    return new Promise((resolve, reject) => {
  ...
  removeTrack(sender: RTCRtpSender) {
    return new Promise((resolve, reject) => {
      ...
      WebRTCModule.peerConnectionRemoveTrackV2(
        this._peerConnectionId,
        theSender.sender.id(),
        (successful) => {
          if (successful) {
  ...
  removeTrackV1(sender: RtpSender) {
    return new Promise((resolve, reject) => {
      ...
      WebRTCModule.peerConnectionRemoveTrack(
        sender.id(),
        this._peerConnectionId,
        (successful) => {
          if (successful) {
  ...
  getStats() {
    return WebRTCModule.peerConnectionGetStats(this._peerConnectionId).then(
      (data) => {
